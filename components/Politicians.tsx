...
const Portfolio: React.FC<{ portfolioItems: PortfolioItem[]; onViewProfile: (id: string) => void }> = ({ portfolioItems, onViewProfile }) => {
  return (
    <section className="m-auto py-16">
      <div
        className="h-[150px] py-[37px] rounded-[24px] flex flex-col items-center justify-center bg-white bg-opacity-20 px-2 mb-20"
        style={{ backgroundColor: "rgba(9, 37, 140, 0.6)" }}
      >
        <h2 className="text-[50px] font-bold text-center mt-6" style={{ fontFamily: "'Inria Serif', serif" }}>
          Candidates
        </h2>
        <h2 className="text-[30px] font-bold text-center mb-8 text-black" style={{ fontFamily: "'Inria Serif', serif" }}>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-24 p-6">
        {portfolioItems.map((item, index) => (
          <PortfolioCard key={index} item={item} onViewProfile={onViewProfile} />
        ))}
      </div>
    </section>
  );
...
const PortfolioCard: React.FC<{ item: PortfolioItem; onViewProfile: (id: string) => void }> = ({ item, onViewProfile }) => {
  return (
