    ...
    jsonParser(req, res, async () => {
        try {
            ...
            if (session.user.role !== "admin") {
                return res.status(403).json({ message: "Forbidden: Only admins can update PLK users" });
            }
            ...
            if (userToUpdate.role !== "plk") {
                return res.status(403).json({ message: "Forbidden: Only PLK users can be updated" });
            }

            // Update fields (including the status field)
            const allowedUpdates = ["name", "email", "phone", "district", "status"]; // Add `status` to the list
