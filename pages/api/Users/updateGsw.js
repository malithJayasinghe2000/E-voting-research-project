    ...
    jsonParser(req, res, async () => {
        try {
            ...
            if (session.user.role !== "plk") {
                return res.status(403).json({ message: "Forbidden: Only plk can update GSW users" });
            }
            ...
            if (userToUpdate.role !== "gsw") {
                return res.status(403).json({ message: "Forbidden: Only gsw users can be updated" });
            }
