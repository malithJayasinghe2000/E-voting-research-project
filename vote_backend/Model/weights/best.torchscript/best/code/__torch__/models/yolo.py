...
class Detect(Module):
  ...
  def forward(self: __torch__.models.yolo.Detect,
    ...
    _18 = torch.view(_14, [_16, 3, 6, _17, int(nx)])
    _19 = torch.contiguous(torch.permute(_18, [0, 1, 3, 4, 2]))
    _20 = torch.split_with_sizes(torch.sigmoid(_19), [2, 2, 2], 4)
    xy, wh, conf, = _20
    ...
    _24 = torch.view(y, [_15, int(_23), 6])
    ...
    _29 = torch.view(_25, [_27, 3, 6, _28, int(nx0)])
    _30 = torch.contiguous(torch.permute(_29, [0, 1, 3, 4, 2]))
    _31 = torch.split_with_sizes(torch.sigmoid(_30), [2, 2, 2], 4)
    xy1, wh1, conf0, = _31
    ...
    _35 = torch.view(y0, [_26, int(_34), 6])
    ...
    _40 = torch.view(_36, [_38, 3, 6, _39, int(nx1)])
    _41 = torch.contiguous(torch.permute(_40, [0, 1, 3, 4, 2]))
    _42 = torch.split_with_sizes(torch.sigmoid(_41), [2, 2, 2], 4)
    xy3, wh3, conf1, = _42
    ...
    _46 = [_24, _35, torch.view(y1, [_37, int(_45), 6])]
